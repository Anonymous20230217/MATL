...
class MLP(nn.Module):
    ...
    def __init__(self, num_i, num_h1, num_h2, num_o):
        ...
        self.linear3 = torch.nn.Linear(num_h2, num_o)
...
class Model(nn.Module):
    ...
    def __init__(self, code_vocab_size, ast_vocab_size, nl_vocab_size,
        ...
        self.MLP = MLP(config.hidden_size + 300, 400, 300, config.hidden_size) #556, 400, 300, 256
        ...
        if config.use_cuda:
            ...
            # self.atten_guide = self.atten_guide.cuda()
            self.atten = self.atten.cuda()
    ...
    def forward(self, batch, batch_size, nl_vocab, is_test=False):
        ...
        """
        # batch: [T, B]
        code_batch, code_seq_lens, ast_batch, ast_seq_lens, attention_mask_batch, nl_batch, nl_seq_lens, name_batch = batch
        ...
        ast_outputs_0, ast_hidden_0 = self.ast_encoder(ast_batch[0], ast_seq_lens[0])
        ast_outputs_1, ast_hidden_1 = self.ast_encoder(ast_batch[1], ast_seq_lens[1])
        ast_outputs_2, ast_hidden_2 = self.ast_encoder(ast_batch[2], ast_seq_lens[2])
        ast_outputs_3, ast_hidden_3 = self.ast_encoder(ast_batch[3], ast_seq_lens[3])
        ast_outputs_4, ast_hidden_4 = self.ast_encoder(ast_batch[4], ast_seq_lens[4])

        ast_outputs = torch.cat((ast_outputs_0, ast_outputs_1, ast_outputs_2, ast_outputs_3, ast_outputs_4), 0)
        # data for decoder
        ...
        # ast_hidden_1 = ast_hidden_1.unsqueeze(0)  # [1, B, H]
        ast_hidden_2 = ast_hidden_2[0] + ast_hidden_2[1]  # [B, H]
        # ast_hidden_2 = ast_hidden_2.unsqueeze(0)  # [1, B, H]
        ast_hidden_3 = ast_hidden_3[0] + ast_hidden_3[1]  # [B, H]
        # ast_hidden_3 = ast_hidden_3.unsqueeze(0)  # [1, B, H]
        ast_hidden_4 = ast_hidden_4[0] + ast_hidden_4[1]  # [B, H]
        # ast_hidden_4 = ast_hidden_4.unsqueeze(0)  # [1, B, H]

        ast_hidden = torch.stack((ast_hidden_0, ast_hidden_1, ast_hidden_2, ast_hidden_3, ast_hidden_4), dim=1)
        ...
        represents = ast_hidden * sent_probs
        represents = represents.sum(dim=1)

        signatures_hiddens = torch.cat((represents, name_batch), 1)
        signatures_hiddens = self.MLP(signatures_hiddens)
        signatures_hiddens = signatures_hiddens.unsqueeze(0)
        # import ipdb
        # ipdb.set_trace()
        decoder_hidden = self.reduce_hidden(code_hidden, signatures_hiddens)  # [1, B, H]
