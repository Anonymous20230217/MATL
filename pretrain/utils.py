...
def filter_data(codes, asts, nls, names,result_vecs):
    """
    ...
    for i in range(len(codes)):
        ...
        new_vecs.append(vec)
    return new_codes, new_asts, new_nls, new_names, new_vecs
...
def unsort_collate_fn(batch, code_vocab, ast_vocab, nl_vocab, raw_nl=False):
    ...
    for b in batch:
        ...
        nl_batch.append(b[2])
        name_batch.append(b[3])
        result_batch.append(b[4])
    ...
    name_batch = indices_from_batch(name_batch, code_vocab)  # [B, T]
    ast_batch, attention_mask_batch = indices_from_batch_for_signature(ast_batch, ast_vocab)  # [B, T]
    if not raw_nl:
        nl_batch = indices_from_batch(nl_batch, nl_vocab)  # [B, T]

    code_seq_lens = get_seq_lens(code_batch)
    name_seq_lens = get_seq_lens(name_batch)

    ast_seq_lens = []
    ast_seq_lens.append(get_seq_lens(ast_batch[0]))
    ast_seq_lens.append(get_seq_lens(ast_batch[1]))
    ast_seq_lens.append(get_seq_lens(ast_batch[2]))
    ast_seq_lens.append(get_seq_lens(ast_batch[3]))
    ast_seq_lens.append(get_seq_lens(ast_batch[4]))

    nl_seq_lens = get_seq_lens(nl_batch)

    # pad and transpose, [T, B], tensor
    code_batch = pad_one_batch(code_batch, code_vocab)
    name_batch = pad_one_batch(name_batch, code_vocab)

    ast_batch[0] = (pad_one_batch(ast_batch[0], ast_vocab))
    ast_batch[1] = (pad_one_batch(ast_batch[1], ast_vocab))
    ast_batch[2] = (pad_one_batch(ast_batch[2], ast_vocab))
    ast_batch[3] = (pad_one_batch(ast_batch[3], ast_vocab))
    ast_batch[4] = (pad_one_batch(ast_batch[4], ast_vocab))

    #print(ast_batch.size)
    #ipdb.set_trace()
    if not raw_nl:
        nl_batch = pad_one_batch(nl_batch, nl_vocab)

    result_batch = torch.tensor(result_batch, device=config.device).float()

    return code_batch, code_seq_lens, \
        ast_batch, ast_seq_lens,  attention_mask_batch,\
        nl_batch, nl_seq_lens, \
        name_batch, name_seq_lens, \
        result_batch
