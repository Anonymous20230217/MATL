...
def train(args):
    ...
    ckpt = f'./output/{args.model}/{args.dataset}/202207271817/models/step4000000.h5'
    ...
    for epoch in range(int(args.reload_from/n_iters)+1, config['nb_epoch']+1): 
        ...
        for batch in data_loader:
            ...
            if global_step % args.valid_every == 0:
                ...
                valid_result = validate(valid_set, model, 10, 1, config['sim_measure'])
...
def validate(valid_set, model, pool_size, K, sim_measure):
    ...
    data_loader = torch.utils.data.DataLoader(dataset=valid_set, batch_size=2,
                                 shuffle=True, drop_last=True, num_workers=1)
...
def parse_args():
    ...
    parser.add_argument('--save_every', type=int, default=100, help='interval to evaluation to concrete results')
